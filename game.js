/**
 * 🍰 Kawaii Bakery — Game Logic (v7 — 50% win condition + audio)
 * game.js
 *
 * Changes from v6 (everything else is untouched):
 *  - WIN CONDITION: the player now wins at 50% of the collectible items,
 *    computed from the live item count (window.BAKERY_TOTAL_ITEMS) rather
 *    than a hardcoded number — add/remove items in items.js and the goal
 *    follows automatically. See getWinTarget().
 *  - HUD shows "placed / goal (50% of N)".
 *  - _won guard so the victory/confetti screen only fires once.
 *  - Correct placement still plays the 'success' confirmation sound; wrong
 *    placement plays 'error' (unchanged).
 *
 * All original desktop + VR gameplay is preserved.
 */
...
class BakeryGame {
  constructor() {
    ...
    // ── WIN CONDITION: 50% of the collectible items ──
    this.WIN_FRACTION = 0.5;
    this._won = false;
  ...
  // ─── WIN TARGET (50% of the live item count) ───────────────────
  getWinTarget() {
    const total = this.totalItems || window.BAKERY_TOTAL_ITEMS || 0;
    if (!total) return 0;
    return Math.max(1, Math.ceil(total * this.WIN_FRACTION));
  }
  ...
  // ─── Physics state transitions (no-ops without a physics system) ─
  ...
  placeItemCorrectly(itemEl, zoneEl) {
    ...
    // Correct-placement confirmation sound (only fires here — never on a
    // wrong placement, which plays 'error' in tryPlace()).
    this._sfx('success');
    ...
    // ── 50% WIN CHECK ──
    const target = this.getWinTarget();
    if (!this._won && target > 0 && this.placedItems >= target) {
      this._won = true;
  ...
  tick() {
    ...
    // Spatial audio listener follows the head (sink / oven / doorway)
    if (window.bakeryAudio && this.camera) {
      window.bakeryAudio.updateListener(this.camera.object3D);
    }

    // Gaze hover highlight — desktop only.
    if (!inVR) {
      try {
  ...
  updateHUD() {
    ...
    const target = this.getWinTarget();
    const pct    = target ? Math.min(100, Math.round((this.placedItems / target) * 100)) : 0;
    document.getElementById('progress-bar').style.width = pct + '%';
    document.getElementById('progress-text').textContent =
      `Items placed: ${this.placedItems} / ${target || '?'}  (50% of ${total || '?'})`;
  }
