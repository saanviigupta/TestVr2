...
window.addEventListener('DOMContentLoaded', () => {
  ...
  scene.addEventListener('loaded', () => {
    ...
    // ── Drop zones ───────────────────────────────────────────────
    // Heights match the LOWERED shelves in index.html:
    //   main shelf boards  → 0.8 / 1.4 / 2.0   (zones at 0.9 / 1.5 / 2.08)
    //   right-wall shelf   → 0.9 / 1.5 / 2.1   (fork + spoon on 0.9 board)
    const extraZones = [
      ...
      { id: 'zone-macaron', type: 'macaron',  pos: '-2.0 1.5 -5.35',  label: '🫐 Macarons'  },
      { id: 'zone-muffin',  type: 'muffin',   pos: '2.0 1.5 -5.35',   label: '🧁 Muffins'   },
      { id: 'zone-brownie', type: 'brownie',  pos: '0 2.08 -5.35',    label: '🍫 Brownies'  },
      { id: 'zone-fork',  type: 'fork',  pos: '5.77 0.98 -0.5', label: '🍴 Forks'  },
      { id: 'zone-spoon', type: 'spoon', pos: '5.77 0.98 -1.5', label: '🥄 Spoons' },
    ];
    ...
    function buildVisual(type) {
      ...
      switch (type) {
        ...
        case 'fork': {
          ...
          // Lay the fork flat (parallel to the floor) instead of standing up.
          group.setAttribute('rotation', '-90 0 0');
          break;
        }
        ...
        case 'spoon': {
          ...
          // Lay the spoon flat (parallel to the floor) instead of standing up.
          group.setAttribute('rotation', '-90 0 0');
          break;
        }
    ...
    itemDefinitions.forEach((def) => {
      for (let i = 0; i < def.count; i++) {
        ...
        // DO NOT SHRINK — this is what makes grabbing forgiving.
