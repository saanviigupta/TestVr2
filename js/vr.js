 ...
 * vr.js
 *
 * ⚠️ UNCHANGED FILE. The grab mechanism, grab radius and snap radius are
 * exactly as they were — grabbing stays as forgiving as it is today.
 * (The purple *test block* was removed from index.html, but the component
 * is still named purple-grab because that is the grab system for EVERY
 * .interactable item. Do not rename it.)
 *
 * Mechanism:
 *   - raw-pose: hand entities driven straight from the XR frame
 *   - purple-grab: ANY button near an item → object3D.attach the item
 *     to the hand; release all buttons → detach in place
 *
 * After a successful release it optionally talks to the game: if the item
 * sits within reach of its matching drop zone, the zone placement + score +
 * sparkles run. If the game code isn't there or errors, grabbing itself
...
/* ─────────────────────────────────────────────
 * PURPLE-GRAB (on <a-scene>) — poll-grab; the grabbable set is every
 * .interactable (all bakery items).
 * ───────────────────────────────────────────── */
  ...
  tick: function () {
    ...
    for (var i = 0; i < session.inputSources.length; i++) {
      ...
      if (pressed && !this._was[hand]) {
        // Rising edge → grab nearest free item
